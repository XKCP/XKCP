...
// INFO: KeccakF1600_StatePermute() execution takes 141596 cycles, tested on ATmega1280 simulator
// INFO: Code size of all functions in this file is 1480 bytes.
...
;// argument laneCount is passed in r20:r21, only LSB (r20) is used
;//
.global KeccakF1600_StateXORLanes
KeccakF1600_StateXORLanes:
	tst		r20
	breq	KeccakF1600_StateXORLanes_End
	movw	rZ, r24
	movw	rX, r22
	lsl		r20 			;//do a half line per iteration
KeccakF1600_StateXORLanes_Loop:
	...
	st		Z+, r0
	dec		r20
	brne	KeccakF1600_StateXORLanes_Loop
KeccakF1600_StateXORLanes_End:
	ret
...
;//
;// argument state        is passed in r24:r25
;// argument lanePosition is passed in r22:r23
;// argument data         is passed in r20:r21
;// argument offset       is passed in r18:r19
...
KeccakF1600_StateXORBytesInLane:
	tst		r16
	breq	KeccakF1600_StateXORBytesInLane_End
	movw	rX, r20
	movw	rZ, r24			;rZ = state + 8 * lanePosition + offset
	lsl		r22
	lsl		r22
	lsl		r22
	add		rZ, r22
...
KeccakF1600_StateXORBytesInLane_Loop:
	...
	st		Z+, r0
	dec		r16
	brne	KeccakF1600_StateXORBytesInLane_Loop
KeccakF1600_StateXORBytesInLane_End:
	ret
...
;// argument state     is passed in r24:r25
;// argument data      is passed in r22:r23
;// argument laneCount is passed in r20:r21
;//
.global KeccakF1600_StateExtractLanes
KeccakF1600_StateExtractLanes:
	tst		r20
	breq	KeccakF1600_StateExtractLanes_End
	movw	rZ, r22
	movw	rX, r24
KeccakF1600_StateExtractLanes_Loop:
	ld		r0, X+
	st		Z+, r0
	ld		r0, X+
	st		Z+, r0
	...
	st		Z+, r0
	dec		r20
	brne	KeccakF1600_StateExtractLanes_Loop
KeccakF1600_StateExtractLanes_End:
	ret
...
;// argument data         is passed in r20:r21
;// argument offset       is passed in r18:r19
;// argument length       is passed in r16:r17
;//
.global KeccakF1600_StateExtractBytesInLane
KeccakF1600_StateExtractBytesInLane:
	tst		r16
	breq	KeccakF1600_StateExtractBytesInLane_End
	movw	rX, r20
	movw	rZ, r24			;rZ = state + 8 * lanePosition + offset
	lsl		r22
	lsl		r22
...
KeccakF1600_StateExtractBytesInLane_Loop:
	...
	st		X+, r0
	dec		r16
	brne	KeccakF1600_StateExtractBytesInLane_Loop
KeccakF1600_StateExtractBytesInLane_End:
	ret
...
;//
;// argument state        is passed in r24:r25
;// argument inData       is passed in r22:r23
;// argument inLaneCount  is passed in r20:r21
;// argument outData      is passed in r18:r19
;// argument outLaneCount is passed in r16:r17
;//
...
KeccakF1600_StateXORPermuteExtract:
	...
	push	r19
	movw	r2, r24
    rcall   KeccakF1600_StateXORLanes

	movw	r24, r2
    rcall   KeccakF1600_StatePermute
	...
	pop		r3
	pop		r2
    rjmp    KeccakF1600_StateExtractLanes
