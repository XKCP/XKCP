...
//__KECCAK_PERMUTE
#if KECCAK_NO_ASM || !(defined(__x86_64__) || defined(__X86_64__) || defined(__LP64__) || \
...
/*********************************/\
#define KECCAK_PERMUTE(const_pref) \
/*********************************/\
...
\
	...
	: "i"(sizeof(keccak_rc)), "d"(keccak_rc), \
...
//*****************************************************************************
{
	...
	SnP_ExtractBytes(state, (UINT8 *)t, 0, sizeof(t));
...
//**************************************************
{
	...
	size_t	lane = position / (SnP_laneLengthInBytes * sizeof(UINT64));
	size_t	bit = position % (SnP_laneLengthInBytes * sizeof(UINT64));
	size_t	y = lane / 5;
	size_t	x = lane % 5;

	if (x <= 3)
		s[y*4 + x] ^= (size_t)1 << bit;	//state.a0..a4
	else
		s[5*4 + y] ^= (size_t)1 << bit;	//state.c4 or a44
} //SnP_ComplementBit
...
//**************************************************************************************************************
{
	...
	for (di = 0; di <= (ptrdiff_t)(dataByteLen / sizeof(UINT64) - laneCount); di += laneCount)
	{
		...
		case 9:		//576
			...
			c4 = XOR(c4, SET(0, trailingBits, 0, 0));
			break;
		...
		case 13:	//832
			...
			a2 = XOR(a2, SET(0, 0, 0, trailingBits));
			break;
		...
		case 16:	//1024
			...
			a3 = XOR(a3, SET(0, trailingBits, 0, 0));
			break;
		...
		case 17:	//1088
			...
			c4 = XOR(c4, SET(d[0*5 + 4], d[1*5 + 4], d[2*5 + 4], 0));
			a3 = XOR(a3, SET(0, 0, trailingBits, 0));
			break;
		...
		case 18:	//1152
			...
			c4 = XOR(c4, SET(d[0*5 + 4], d[1*5 + 4], d[2*5 + 4], 0));
			a3 = XOR(a3, SET(0, 0, 0, trailingBits));
			break;
		...
		case 21:	//1344
			...
			a4 = XOR(a4, SET(0, trailingBits, 0, 0));
			break;
		...
		default:
			...
			// "trailingBits + 256" is passed as offset to do "state ^ trailingBits".
			SnP_XORBytes(state, (UINT8 *)d, (size_t)trailingBits + 256, laneCount * sizeof(UINT64));
...
//*************************************************************************************
{
	...
	UINT64		*d, *p;
	UINT64		t[25];
	ptrdiff_t	end0 = dataByteLen / sizeof(UINT64) - SnP_laneCount;
	ptrdiff_t	end = dataByteLen / sizeof(UINT64) - laneCount;
	ptrdiff_t	di, i;

	KECCAK_LOAD

	for (di = 0; di <= end; di += laneCount)
	{
		KECCAK_PERMUTE(_)
		...
		default:
			...
			if (di > end0)
				for (i = 0; i < (ptrdiff_t)laneCount; i++)
					d[i] = p[i];
		} //switch (laneCount)
	} //for (di
...
//******************************************************************************************************************************
{
	...
	for (di = 0; di <= (ptrdiff_t)(dataByteLen / sizeof(UINT64) - laneCount); di += laneCount)
	{
		...
		default:
			bytes = laneCount * SnP_laneLengthInBytes;

			for (di = 0; di <= (ptrdiff_t)(dataByteLen - bytes); di += bytes)
			{
				SnP_XORBytes(state, dataIn + di, 0, bytes);
				SnP_ExtractBytes(state, dataOut + di, 0, bytes);
				SnP_XORBytes(state, &trailingBits, bytes, 1);
				SnP_Permute(state);
			}
...
//********************************************************************************************************************************
{
	...
	for (di = 0; di <= (ptrdiff_t)(dataByteLen / sizeof(UINT64) - laneCount); di += laneCount)
	{
		...
		case 9:		//576
			b0 = LOAD(d + 0*5);
			b1 = LOAD(d + 1*5);
			d4 = MASKLOAD(d + 0*5 + 4, 1, 0, 0, 0);

			STORE(t + 0*5, XOR(a0, b0));
			STORE(t + 1*5, XOR(a1, b1));
			STORE0(t + 0*5 + 4, XOR(c4, d4));

			a0 = b0;
			a1 = b1;
			c4 = BLEND(c4, d4, 1, 0, 0, 0);
			c4 = XOR(c4, SET(0, trailingBits, 0, 0));
			break;
		...
		case 13:	//832
			b0 = LOAD(d + 0*5);
			b1 = LOAD(d + 1*5);
			b2 = MASKLOAD(d + 2*5, 1, 1, 1, 0);
			d4 = SET(d[0*5 + 4], d[1*5 + 4], 0, 0);

			STORE(t + 0*5, XOR(a0, b0));
			STORE(t + 1*5, XOR(a1, b1));
			MASKSTORE(t + 2*5, 1, 1, 1, 0, XOR(a2, b2));

			STORE0(t + 0*5 + 4, XOR(c4, d4));
			STORE1(t + 1*5 + 4, XOR(c4, d4));

			a0 = b0;
			a1 = b1;
			a2 = BLEND(a2, b2, 1, 1, 1, 0);
			c4 = BLEND(c4, d4, 1, 1, 0, 0);
			a2 = XOR(a2, SET(0, 0, 0, trailingBits));
			break;
		...
		case 16:	//1024
			...
			b1 = LOAD(d + 1*5);
			b2 = LOAD(d + 2*5);
			b3 = MASKLOAD(d + 3*5, 1, 0, 0, 0);
			d4 = SET(d[0*5 + 4], d[1*5 + 4], d[2*5 + 4], 0);

			STORE(t + 0*5, XOR(a0, b0));
			STORE(t + 1*5, XOR(a1, b1));
			STORE(t + 2*5, XOR(a2, b2));
			MASKSTORE(t + 3*5, 1, 0, 0, 0, XOR(a3, b3));

			STORE0(t + 0*5 + 4, XOR(c4, d4));
			STORE1(t + 1*5 + 4, XOR(c4, d4));
			STORE2(t + 2*5 + 4, XOR(c4, d4));

			a0 = b0;
			a1 = b1;
			a2 = b2;
			a3 = BLEND(a3, b3, 1, 0, 0, 0);
			c4 = BLEND(c4, d4, 1, 1, 1, 0);
		...
		case 17:	//1088
			...
			b1 = LOAD(d + 1*5);
			b2 = LOAD(d + 2*5);
			b3 = MASKLOAD(d + 3*5, 1, 1, 0, 0);
			d4 = SET(d[0*5 + 4], d[1*5 + 4], d[2*5 + 4], 0);

			STORE(t + 0*5, XOR(a0, b0));
			STORE(t + 1*5, XOR(a1, b1));
			STORE(t + 2*5, XOR(a2, b2));
			MASKSTORE(t + 3*5, 1, 1, 0, 0, XOR(a3, b3));
		...
		case 18:	//1152
			...
			b1 = LOAD(d + 1*5);
			b2 = LOAD(d + 2*5);
			b3 = MASKLOAD(d + 3*5, 1, 1, 1, 0);
			d4 = SET(d[0*5 + 4], d[1*5 + 4], d[2*5 + 4], 0);

			STORE(t + 0*5, XOR(a0, b0));
			STORE(t + 1*5, XOR(a1, b1));
			STORE(t + 2*5, XOR(a2, b2));
			MASKSTORE(t + 3*5, 1, 1, 1, 0, XOR(a3, b3));
		...
		case 21:	//1344
			...
			b2 = LOAD(d + 2*5);
			b3 = LOAD(d + 3*5);
			b4 = MASKLOAD(d + 4*5, 1, 0, 0, 0);
			d4 = SET(d[0*5 + 4], d[1*5 + 4], d[2*5 + 4], d[3*5 + 4]);

			STORE(t + 0*5, XOR(a0, b0));
			STORE(t + 1*5, XOR(a1, b1));
			STORE(t + 2*5, XOR(a2, b2));
			STORE(t + 3*5, XOR(a3, b3));
			MASKSTORE(t + 4*5, 1, 0, 0, 0, XOR(a4, b4));

			STORE0(t + 0*5 + 4, XOR(c4, d4));
			STORE1(t + 1*5 + 4, XOR(c4, d4));
			STORE2(t + 2*5 + 4, XOR(c4, d4));
			STORE3(t + 3*5 + 4, XOR(c4, d4));

			a0 = b0;
			a1 = b1;
			a2 = b2;
			a3 = b3;
			a4 = BLEND(a4, b4, 1, 0, 0, 0);
			c4 = d4;
		...
		case 25:	//1600
			...
			STORE(t + 2*5, XOR(a2, b2));
			STORE(t + 3*5, XOR(a3, b3));
			STORE(t + 4*5, XOR(a4, b4));

			STORE0(t + 0*5 + 4, XOR(c4, d4));
			STORE1(t + 1*5 + 4, XOR(c4, d4));
			STORE2(t + 2*5 + 4, XOR(c4, d4));
			STORE3(t + 3*5 + 4, XOR(c4, d4));
			STORE0(t + 4*5 + 4, XOR(a44, b44));
		...
		default:
			...
			for (di = 0; di <= (ptrdiff_t)(dataByteLen - bytes); di += bytes)
			{
				SnP_ExtractAndXORBytes(state, dataOut + di, 0, bytes);
				SnP_XORBytes(state, dataOut + di, trailingBits + 256, bytes);
				SnP_Permute(state);
			}
