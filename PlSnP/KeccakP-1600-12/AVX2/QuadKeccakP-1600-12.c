...
#define laneIndex(instanceIndex, lanePosition) ((lanePosition)*LaneInterleaving + instanceIndex)
...
const UINT64 rho8[4] = {0x0605040302010007, 0x0E0D0C0B0A09080F, 0x1615141312111017, 0x1E1D1C1B1A19181F};
const UINT64 rho56[4] = {0x0007060504030201, 0x080F0E0D0C0B0A09, 0x1017161514131211, 0x181F1E1D1C1B1A19};
...
void KeccakF1600_Pl4_StaticInitialize( void )
{
}

void KeccakF1600_Pl4_InitializeAll(void *states)
{
    memset(states, 0, PlSnP_statesSizeInBytes);
}

void KeccakF1600_Pl4_XORBytes(void *states, unsigned int instanceIndex, const unsigned char *data, unsigned int offset, unsigned int length)
{
...
void KeccakF1600_Pl4_XORLanesAll(void *states, const unsigned char *data, unsigned int laneCount, unsigned int laneOffset)
{
...
void KeccakF1600_Pl4_OverwriteBytes(void *states, unsigned int instanceIndex, const unsigned char *data, unsigned int offset, unsigned int length)
{
    unsigned int sizeLeft = length;
    unsigned int lanePosition = offset/SnP_laneLengthInBytes;
    unsigned int offsetInLane = offset%SnP_laneLengthInBytes;
...
void KeccakF1600_Pl4_OverwriteLanesAll(void *states, const unsigned char *data, unsigned int laneCount, unsigned int laneOffset)
{
    V256 *stateAsLanes = states;
    unsigned int i;
    const UINT64 *curData0 = (const UINT64 *)data;
...
void KeccakF1600_Pl4_OverwriteWithZeroes(void *states, unsigned int instanceIndex, unsigned int byteCount)
{
...
void KeccakF1600_Pl4_ComplementBit(void *states, unsigned int instanceIndex, unsigned int position)
{
    unsigned int lanePosition = position/(SnP_laneLengthInBytes*8);
    UINT64 bitMask = (UINT64)1 << (position%(SnP_laneLengthInBytes*8));
    UINT64 *statesAsLanes = states;

	statesAsLanes[laneIndex(instanceIndex, lanePosition)] ^= bitMask;
}

void KeccakF1600_Pl4_ComplementBitAll(void *states, unsigned int position)
{
    unsigned int lanePosition = position/(SnP_laneLengthInBytes*8);
    UINT64 bitMask = (UINT64)1 << (position%(SnP_laneLengthInBytes*8));
    UINT64 *statesAsLanes = states;

	statesAsLanes[laneIndex(0, lanePosition)] ^= bitMask;
	statesAsLanes[laneIndex(1, lanePosition)] ^= bitMask;
	statesAsLanes[laneIndex(2, lanePosition)] ^= bitMask;
	statesAsLanes[laneIndex(3, lanePosition)] ^= bitMask;
}

void KeccakF1600_Pl4_ExtractBytes(const void *states, unsigned int instanceIndex, unsigned char *data, unsigned int offset, unsigned int length)
{
    unsigned int sizeLeft = length;
    unsigned int lanePosition = offset/SnP_laneLengthInBytes;
    unsigned int offsetInLane = offset%SnP_laneLengthInBytes;
...
void KeccakF1600_Pl4_ExtractLanesAll(const void *states, unsigned char *data, unsigned int laneCount, unsigned int laneOffset)
{
...
void KeccakF1600_Pl4_ExtractAndXORBytes(const void *states, unsigned int instanceIndex,  unsigned char *data, unsigned int offset, unsigned int length)
{
    unsigned int sizeLeft = length;
    unsigned int lanePosition = offset/SnP_laneLengthInBytes;
    unsigned int offsetInLane = offset%SnP_laneLengthInBytes;
...
void KeccakF1600_Pl4_ExtractAndXORLanesAll(const void *states, unsigned char *data, unsigned int laneCount, unsigned int laneOffset)
{
    UINT64 *curData0 = (UINT64 *)data;
    UINT64 *curData1 = (UINT64 *)(data+laneOffset*1*SnP_laneLengthInBytes);
    UINT64 *curData2 = (UINT64 *)(data+laneOffset*2*SnP_laneLengthInBytes);
	...
	#define	ExtrXor( argIndex )	curData0[argIndex] ^= stateAsLanes64[4*(argIndex)],\
								curData1[argIndex] ^= stateAsLanes64[4*(argIndex)+1],\
								curData2[argIndex] ^= stateAsLanes64[4*(argIndex)+2],\
								curData3[argIndex] ^= stateAsLanes64[4*(argIndex)+3]

	#define	ExtrXor4( argIndex )	lanes0 = LOAD256( stateAsLanes[argIndex+0] ),\
									lanes1 = LOAD256( stateAsLanes[argIndex+1] ),\
									lanes2 = LOAD256( stateAsLanes[argIndex+2] ),\
									lanes3 = LOAD256( stateAsLanes[argIndex+3] ),\
									UNINTLEAVE(),\
									lanesL01 = LOAD256u( curData0[argIndex]),\
									lanesH01 = LOAD256u( curData1[argIndex]),\
									lanesL23 = LOAD256u( curData2[argIndex]),\
									lanesH23 = LOAD256u( curData3[argIndex]),\
...
/*
size_t KeccakF1600_Pl4_FBWL_Absorb(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, unsigned char *data, size_t dataByteLen, unsigned char trailingBits)
{
}

size_t KeccakF1600_Pl4_FBWL_Squeeze(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, unsigned char *data, size_t dataByteLen)
{
}

size_t KeccakF1600_Pl4_FBWL_Wrap(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, const unsigned char *dataIn, unsigned char *dataOut, size_t dataByteLen, unsigned char trailingBits)
{
}

size_t KeccakF1600_Pl4_FBWL_Unwrap(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, const unsigned char *dataIn, unsigned char *dataOut, size_t dataByteLen, unsigned char trailingBits)
{
}
*/
...
ALIGN const UINT64 KeccakF1600RoundConstants[12] = {
...
void KeccakP1600_12_Pl4_PermuteAll(void *states)
{
...
void KeccakP1600_12_Pl4_Permute(void *states, unsigned int instanceIndex)
{
    ALIGN UINT64 tempStates[PlSnP_statesSizeInBytes / sizeof(UINT64)];
    UINT64 *statesAsLanes = states;
    unsigned int i;

	memcpy( tempStates, states, PlSnP_statesSizeInBytes );
	KeccakP1600_12_Pl4_PermuteAll( tempStates );
	for ( i = 0; i < SnP_laneCount; ++i ) {
        statesAsLanes[laneIndex(instanceIndex, i)] = tempStates[laneIndex(instanceIndex, i)];
	}
}
