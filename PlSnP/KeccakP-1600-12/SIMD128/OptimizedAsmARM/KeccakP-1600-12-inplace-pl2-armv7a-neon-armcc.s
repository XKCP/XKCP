...
; --- macros for Parallel permutation
    ...
    ; argA1 = Ba ^(~Be & Bi) ^ KeccakF1600RoundConstants[round]
    ; argA2 = Be ^(~Bi & Bo)
    ; argA3 = Bi ^(~Bo & Bu)
    ; argA4 = Bo ^(~Bu & Ba)
    ; argA5 = Bu ^(~Ba & Be)
...
;----------------------------------------------------------------------------
;
; void KeccakF1600_Pl_Initialize( void )
;
    ALIGN
    EXPORT  KeccakF1600_Pl_Initialize
KeccakF1600_Pl_Initialize   PROC
    bx		lr
    ENDP

;----------------------------------------------------------------------------
;
; void KeccakF1600_Pl2_InitializeAll( void *states )
;
    ALIGN
    EXPORT  KeccakF1600_Pl2_InitializeAll
KeccakF1600_Pl2_InitializeAll   PROC
    vmov.i64	q0, #0
    vmov.i64	q1, #0
    vmov.i64	q2, #0
    vmov.i64	q3, #0
...
;----------------------------------------------------------------------------
;
; void KeccakF1600_Pl2_ComplementBit( void *states, unsigned int instanceIndex, unsigned int position )
;
    ALIGN
    EXPORT	KeccakF1600_Pl2_ComplementBit
KeccakF1600_Pl2_ComplementBit   PROC
    add		r0, r0, r1, LSL #3			; states += 8 * instanceIndex
    lsr		r1, r2, #6					; states += position / 64 * 2 * 8
    add		r0, r0, r1, LSL #4
    lsr 	r1, r2, #5					; r1 = (position / 32) & 1
    and		r1, r1, #1
...
;----------------------------------------------------------------------------
;
; void KeccakF1600_Pl2_ComplementBitAll( void *states, unsigned int position )
;
    ALIGN
    EXPORT	KeccakF1600_Pl2_ComplementBitAll
KeccakF1600_Pl2_ComplementBitAll   PROC
...
;
    ALIGN
    EXPORT  KeccakF1600_Pl2_XORBytes
KeccakF1600_Pl2_XORBytes   PROC
    add		r0, r0, r1, LSL #3			; states += 8 * instanceIndex
    ldr		r1, [sp, #0*4]				; r1 = length
    cmp		r1, #0
    beq		KeccakF1600_Pl2_XORBytes_Exit
    push	{ r4- r7 }
    lsr		r4, r3, #3					; states += (offset & ~7) * 2
    add		r0, r0, r4, LSL #4
    ands	r3, r3, #7					; if (offset & 7) != 0
    beq		KeccakF1600_Pl2_XORBytes_CheckLanes
    add		r0, r0, r3					; states += offset & 7
    rsb		r3, r3, #8					; lenInLane = 8 - (offset & 7)
KeccakF1600_Pl2_XORBytes_LoopBytesFirst
    ldrb	r4, [r0]
    ldrb	r5, [r2], #1
    eor		r4, r4, r5
    subs	r1, r1, #1
    strb	r4, [r0], #1
    beq		KeccakF1600_Pl2_XORBytes_Done
    subs	r3, r3, #1
    bne		KeccakF1600_Pl2_XORBytes_LoopBytesFirst
    add		r0, r0, #8					; states += 8 (next lane of current state part)
KeccakF1600_Pl2_XORBytes_CheckLanes
    lsrs	r3, r1, #3
    beq		KeccakF1600_Pl2_XORBytes_CheckBytesLast
KeccakF1600_Pl2_XORBytes_LoopLanes
    ldr		r4, [r0]
    ldr		r5, [r0, #4]
    ldr		r6, [r2], #4
    ldr		r7, [r2], #4
    ...
    bne		KeccakF1600_Pl2_XORBytes_LoopLanes
KeccakF1600_Pl2_XORBytes_CheckBytesLast
    ands	r1, r1, #7
    beq		KeccakF1600_Pl2_XORBytes_Done
KeccakF1600_Pl2_XORBytes_LoopBytesLast
    ldrb	r4, [r0]
    ldrb	r5, [r2], #1
    eor		r4, r4, r5
    subs	r1, r1, #1
    strb	r4, [r0], #1
    bne		KeccakF1600_Pl2_XORBytes_LoopBytesLast
KeccakF1600_Pl2_XORBytes_Done
    pop		{ r4- r7 }
...
;----------------------------------------------------------------------------
;
; void KeccakF1600_Pl2_XORLanesAll( void *states, const unsigned char *data, unsigned int laneCount, unsigned int laneOffset )
;
    EXPORT  KeccakF1600_Pl2_XORLanesAll
    ALIGN
KeccakF1600_Pl2_XORLanesAll   PROC
    cmp		r2, #0
    beq		KeccakF1600_Pl2_XORLanesAll_Exit
    add		r3, r1, r3, LSL #3		; r3: data + 8 * laneOffset
    push	{r4 - r7}
KeccakF1600_Pl2_XORLanesAll_Loop
    ldr		r4, [r1], #4			; index 0
    ldr		r5, [r1], #4
    ldrd    r6, r7, [r0]
    eor		r6, r6, r4
    ...
    strd	r6, r7, [r0], #8
    subs	r2, r2, #1
    bne		KeccakF1600_Pl2_XORLanesAll_Loop
    pop		{r4 - r7}
...
;
    ALIGN
    EXPORT	KeccakF1600_Pl2_OverwriteBytes
KeccakF1600_Pl2_OverwriteBytes	PROC
    add		r0, r0, r1, LSL #3			; states += 8 * instanceIndex
    ldr		r1, [sp, #0*4]				; r1 = length
    cmp		r1, #0
    beq		KeccakF1600_Pl2_OverwriteBytes_Exit
    push	{ r4-r5 }
    lsr		r4, r3, #3					; states += (offset & ~7) * 2
    add		r0, r0, r4, LSL #4
    ands	r3, r3, #7					; if (offset & 7) != 0
    beq		KeccakF1600_Pl2_OverwriteBytes_CheckLanes
    add		r0, r0, r3					; states += offset & 7
    rsb		r3, r3, #8					; lenInLane = 8 - (offset & 7)
KeccakF1600_Pl2_OverwriteBytes_LoopBytesFirst
    ldrb	r4, [r2], #1
    strb	r4, [r0], #1
    subs	r1, r1, #1
    beq		KeccakF1600_Pl2_OverwriteBytes_Done
    subs	r3, r3, #1
    bne		KeccakF1600_Pl2_OverwriteBytes_LoopBytesFirst
    add		r0, r0, #8					; states += 8 (next lane of current state part)
KeccakF1600_Pl2_OverwriteBytes_CheckLanes
    lsrs	r3, r1, #3
    beq		KeccakF1600_Pl2_OverwriteBytes_CheckBytesLast
KeccakF1600_Pl2_OverwriteBytes_LoopLanes
    ldr		r4, [r2], #4
    ldr		r5, [r2], #4
    str		r4, [r0], #4
    str		r5, [r0], #12				; states += 8 (next lane of current state part)
    subs	r3, r3, #1
    bne		KeccakF1600_Pl2_OverwriteBytes_LoopLanes
KeccakF1600_Pl2_OverwriteBytes_CheckBytesLast
    ands	r1, r1, #7
    beq		KeccakF1600_Pl2_OverwriteBytes_Done
KeccakF1600_Pl2_OverwriteBytes_LoopBytesLast
    ldrb	r4, [r2], #1
    subs	r1, r1, #1
    strb	r4, [r0], #1
    bne		KeccakF1600_Pl2_OverwriteBytes_LoopBytesLast
KeccakF1600_Pl2_OverwriteBytes_Done
    pop		{ r4- r5 }
...
;----------------------------------------------------------------------------
;
; KeccakF1600_Pl2_OverwriteLanesAll( void *states, const unsigned char *data, unsigned int laneCount, unsigned int laneOffset )
;
    ALIGN
    EXPORT	KeccakF1600_Pl2_OverwriteLanesAll
KeccakF1600_Pl2_OverwriteLanesAll	PROC
    cmp		r2, #0
    beq		KeccakF1600_Pl2_OverwriteLanesAll_Exit
    lsls	r12, r1, #32-3
    bne		KeccakF1600_Pl2_OverwriteLanesAll_Unaligned
    add		r3, r1, r3, LSL #3		; r3(pointer instance 1): data + 8 * laneOffset
    lsrs	r2, r2, #1
    bcc		KeccakF1600_Pl2_OverwriteLanesAll_LoopAligned
    vldm	r1!, { d0 }
    vldm	r3!, { d1 }
    vstm	r0!, { d0 - d1 }
    beq		KeccakF1600_Pl2_OverwriteLanesAll_Exit
KeccakF1600_Pl2_OverwriteLanesAll_LoopAligned
    vldm	r1!, { d0 }
    vldm	r1!, { d2 }
    vldm	r3!, { d1 }
    vldm	r3!, { d3 }
    subs	r2, r2, #1
    vstm	r0!, { d0 - d3 }
    bne		KeccakF1600_Pl2_OverwriteLanesAll_LoopAligned
    bx		lr
KeccakF1600_Pl2_OverwriteLanesAll_Unaligned
    add		r3, r1, r3, LSL #3		; r3(pointer instance 1): data + 8 * laneOffset
    push	{ r4, r5 }
KeccakF1600_Pl2_OverwriteLanesAll_LoopUnaligned
    ldr		r4, [r1], #4
    ldr		r5, [r1], #4
    strd	r4, r5, [r0], #8
    ldr		r4, [r3], #4
    ...
    subs	r2, r2, #1
    strd	r4, r5, [r0], #8
    bne		KeccakF1600_Pl2_OverwriteLanesAll_LoopUnaligned
    pop		{ r4, r5 }
...
;----------------------------------------------------------------------------
;
; void KeccakF1600_Pl2_OverwriteWithZeroes( void *states, unsigned int instanceIndex, unsigned int byteCount )
;
    ALIGN
    EXPORT	KeccakF1600_Pl2_OverwriteWithZeroes
KeccakF1600_Pl2_OverwriteWithZeroes	PROC
    add		r0, r0, r1, LSL #3			; states += 8 * instanceIndex
...
KeccakF1600_Pl2_OverwriteWithZeroes_Bytes
    ...
    beq		KeccakF1600_Pl2_OverwriteWithZeroes_Exit
    movs	r3, #0
KeccakF1600_Pl2_OverwriteWithZeroes_LoopBytes
    subs	r2, r2, #1
    strb	r3, [r0], #1
...
;
    ALIGN
    EXPORT	KeccakF1600_Pl2_ExtractBytes
KeccakF1600_Pl2_ExtractBytes   PROC
    add		r0, r0, r1, LSL #3			; states += 8 * instanceIndex
    ldr		r1, [sp, #0*4]				; r1 = length
    cmp		r1, #0
    beq		KeccakF1600_Pl2_ExtractBytes_Exit
    push	{ r4-r5 }
    lsr		r4, r3, #3					; states += (offset & ~7) * 2
    add		r0, r0, r4, LSL #4
    ands	r3, r3, #7					; if (offset & 7) != 0
    beq		KeccakF1600_Pl2_ExtractBytes_CheckLanes
    add		r0, r0, r3					; states += offset & 7
    rsb		r3, r3, #8					; lenInLane = 8 - (offset & 7)
KeccakF1600_Pl2_ExtractBytes_LoopBytesFirst
    ldrb	r4, [r0], #1
    strb	r4, [r2], #1
    subs	r1, r1, #1
    beq		KeccakF1600_Pl2_ExtractBytes_Done
    subs	r3, r3, #1
    bne		KeccakF1600_Pl2_ExtractBytes_LoopBytesFirst
    add		r0, r0, #8					; states += 8 (next lane of current state part)
KeccakF1600_Pl2_ExtractBytes_CheckLanes
    lsrs	r3, r1, #3
    beq		KeccakF1600_Pl2_ExtractBytes_CheckBytesLast
KeccakF1600_Pl2_ExtractBytes_LoopLanes
    ldr		r4, [r0], #4
    ldr		r5, [r0], #12				; states += 8 (next lane of current state part)
    str		r4, [r2], #4
    str		r5, [r2], #4
    subs	r3, r3, #1
    bne		KeccakF1600_Pl2_ExtractBytes_LoopLanes
KeccakF1600_Pl2_ExtractBytes_CheckBytesLast
    ands	r1, r1, #7
    beq		KeccakF1600_Pl2_ExtractBytes_Done
KeccakF1600_Pl2_ExtractBytes_LoopBytesLast
    ldrb	r4, [r0], #1
    subs	r1, r1, #1
    strb	r4, [r2], #1
    bne		KeccakF1600_Pl2_ExtractBytes_LoopBytesLast
KeccakF1600_Pl2_ExtractBytes_Done
    pop		{ r4-r5 }
...
;
    ALIGN
    EXPORT	KeccakF1600_Pl2_ExtractLanesAll
KeccakF1600_Pl2_ExtractLanesAll   PROC
    cmp		r2, #0
    beq		KeccakF1600_Pl2_ExtractLanesAll_Exit
    lsls	r12, r1, #32-3
    bne		KeccakF1600_Pl2_ExtractLanesAll_Unaligned
    add		r3, r1, r3, LSL #3		; r3(pointer instance 1): data + 8 * laneOffset
    lsrs	r2, r2, #1
    bcc		KeccakF1600_Pl2_ExtractLanesAll_LoopAligned
    vldm	r0!, { d0 - d1 }
    vstm	r1!, { d0 }
    vstm	r3!, { d1 }
    beq		KeccakF1600_Pl2_ExtractLanesAll_Exit
KeccakF1600_Pl2_ExtractLanesAll_LoopAligned
    vldm	r0!, { d0 - d3 }
    subs	r2, r2, #1
    vstm	r1!, { d0 }
    vstm	r1!, { d2 }
    vstm	r3!, { d1 }
    vstm	r3!, { d3 }
    bne		KeccakF1600_Pl2_ExtractLanesAll_LoopAligned
    bx		lr
KeccakF1600_Pl2_ExtractLanesAll_Unaligned
    add		r3, r1, r3, LSL #3		; r3(pointer instance 1): data + 8 * laneOffset
    push	{ r4, r5 }
KeccakF1600_Pl2_ExtractLanesAll_LoopUnaligned
    ldrd	r4, r5, [r0], #8
    str		r4, [r1], #4
    str		r5, [r1], #4
    ldrd	r4, r5, [r0], #8
    ...
    str		r4, [r3], #4
    str		r5, [r3], #4
    bne		KeccakF1600_Pl2_ExtractLanesAll_LoopUnaligned
    pop		{ r4, r5 }
...
;
    ALIGN
    EXPORT	KeccakF1600_Pl2_ExtractAndXORBytes
KeccakF1600_Pl2_ExtractAndXORBytes   PROC
    add		r0, r0, r1, LSL #3			; states += 8 * instanceIndex
    ldr		r1, [sp, #0*4]				; r1 = length
    cmp		r1, #0
    beq		KeccakF1600_Pl2_ExtractAndXORBytes_Exit
    push	{ r4- r7 }
    lsr		r4, r3, #3					; states += (offset & ~7) * 2
    add		r0, r0, r4, LSL #4
    ands	r3, r3, #7					; if (offset & 7) != 0
    beq		KeccakF1600_Pl2_ExtractAndXORBytes_CheckLanes
    add		r0, r0, r3					; states += offset & 7
    rsb		r3, r3, #8					; lenInLane = 8 - (offset & 7)
KeccakF1600_Pl2_ExtractAndXORBytes_LoopBytesFirst
    ldrb	r4, [r0], #1
    ldrb	r5, [r2]
    eor 	r4, r4, r5
    strb	r4, [r2], #1
    subs	r1, r1, #1
    beq		KeccakF1600_Pl2_ExtractAndXORBytes_Done
    subs	r3, r3, #1
    bne		KeccakF1600_Pl2_ExtractAndXORBytes_LoopBytesFirst
    add		r0, r0, #8					; states += 8 (next lane of current state part)
KeccakF1600_Pl2_ExtractAndXORBytes_CheckLanes
    lsrs	r3, r1, #3
    beq		KeccakF1600_Pl2_ExtractAndXORBytes_CheckBytesLast
KeccakF1600_Pl2_ExtractAndXORBytes_LoopLanes
    ldr		r4, [r0], #4
    ldr		r5, [r0], #12
    ldr		r6, [r2]
    ldr		r7, [r2, #4]
    ...
    eor 	r5, r5, r7
    str		r4, [r2], #4
    str		r5, [r2], #4				; states += 8 (next lane of current state part)
    subs	r3, r3, #1
    bne		KeccakF1600_Pl2_ExtractAndXORBytes_LoopLanes
KeccakF1600_Pl2_ExtractAndXORBytes_CheckBytesLast
    ands	r1, r1, #7
    beq		KeccakF1600_Pl2_ExtractAndXORBytes_Done
KeccakF1600_Pl2_ExtractAndXORBytes_LoopBytesLast
    ldrb	r4, [r0], #1
    ldrb	r5, [r2]
    eor 	r4, r4, r5
    strb	r4, [r2], #1
    subs	r1, r1, #1
    bne		KeccakF1600_Pl2_ExtractAndXORBytes_LoopBytesLast
KeccakF1600_Pl2_ExtractAndXORBytes_Done
    pop		{ r4- r7 }
...
;----------------------------------------------------------------------------
;
; void KeccakF1600_Pl2_ExtractAndXORLanesAll( const void *states, unsigned char *data, unsigned int laneCount, unsigned int laneOffset )
;
    ALIGN
    EXPORT	KeccakF1600_Pl2_ExtractAndXORLanesAll
KeccakF1600_Pl2_ExtractAndXORLanesAll	PROC
    cmp		r2, #0
    beq		KeccakF1600_Pl2_ExtractAndXORLanesAll_Exit
    lsls	r12, r1, #32-3
    bne		KeccakF1600_Pl2_ExtractAndXORLanesAll_Unaligned
    add		r3, r1, r3, LSL #3		; r3(pointer instance 1): data + 8 * laneOffset
    lsrs	r2, r2, #1
    bcc		KeccakF1600_Pl2_ExtractAndXORLanesAll_LoopAligned
    vldm	r0!, { d0 - d1 }
    vldm	r1, { d2 }
    vldm	r3, { d3 }
    veor	q0, q0, q1
    ...
    vstm	r3!, { d1 }
    beq		KeccakF1600_Pl2_ExtractAndXORLanesAll_Exit
KeccakF1600_Pl2_ExtractAndXORLanesAll_LoopAligned
    vldm	r0!, { d0 - d3 }
    vldm	r1!, { d4 }
    vldm	r1, { d6 }
    vldm	r3!, { d5 }
    ...
    vstm	r3!, { d1 }
    vstm	r3!, { d3 }
    bne		KeccakF1600_Pl2_ExtractAndXORLanesAll_LoopAligned
    bx		lr
KeccakF1600_Pl2_ExtractAndXORLanesAll_Unaligned
    add		r3, r1, r3, LSL #3		; r3(pointer instance 1): data + 8 * laneOffset
    push	{ r4 - r7 }
KeccakF1600_Pl2_ExtractAndXORLanesAll_LoopUnaligned
    ldrd	r4, r5, [r0], #8
    ldr		r6, [r1]
    ldr		r7, [r1, #4]
    eor		r4, r4, r6
    ...
    subs	r2, r2, #1
    str		r5, [r3], #4
    bne		KeccakF1600_Pl2_ExtractAndXORLanesAll_LoopUnaligned
    pop		{ r4 - r7 }
...
;----------------------------------------------------------------------------
;
; void KeccakP1600_12_Pl2_Permute( void *states, unsigned int instanceIndex )
;
    ALIGN
    EXPORT	KeccakP1600_12_Pl2_Permute
KeccakP1600_12_Pl2_Permute   PROC
    add		r0, r0, r1, LSL #3			; states += 8 * instanceIndex
    ...
    veor.64 d5, d0,  d1
    veor.64 d5, d5,  d4
KeccakP1600_12_ParallelPermute_RoundLoop
    KeccakS_ThetaRhoPiChiIota     _ba, d13, d19, d25, d31
    KeccakS_ThetaRhoPiChi1        _ka, d15, d21, d22, d28
    KeccakS_ThetaRhoPiChi2        _sa, d12, d18, d24, d30
    KeccakS_ThetaRhoPiChi3        _ga, d14, d20, d26, d27
    KeccakS_ThetaRhoPiChi4        _ma, d16, d17, d23, d29
    KeccakS_ThetaRhoPiChiIota     _ba, d15, d18, d26, d29
    KeccakS_ThetaRhoPiChi1        _sa, d14, d17, d25, d28
    KeccakS_ThetaRhoPiChi2        _ma, d13, d21, d24, d27
    KeccakS_ThetaRhoPiChi3        _ka, d12, d20, d23, d31
    KeccakS_ThetaRhoPiChi4        _ga, d16, d19, d22, d30
    KeccakS_ThetaRhoPiChiIota     _ba, d14, d21, d23, d30
    KeccakS_ThetaRhoPiChi1        _ma, d12, d19, d26, d28
    KeccakS_ThetaRhoPiChi2        _ga, d15, d17, d24, d31
    KeccakS_ThetaRhoPiChi3        _sa, d13, d20, d22, d29
    KeccakS_ThetaRhoPiChi4        _ka, d16, d18, d25, d27
    KeccakS_ThetaRhoPiChiIota     _ba, d12, d17, d22, d27
    KeccakS_ThetaRhoPiChi1        _ga, d13, d18, d23, d28
    KeccakS_ThetaRhoPiChi2        _ka, d14, d19, d24, d29
    KeccakS_ThetaRhoPiChi3        _ma, d15, d20, d25, d30
    subs                          r2, #4
    KeccakS_ThetaRhoPiChi4        _sa, d16, d21, d26, d31
    bne                           KeccakP1600_12_ParallelPermute_RoundLoop
    vstr.64 d12, [r0, #_be]
    vstr.64 d17, [r0, #_bi]
    vstr.64 d22, [r0, #_bo]
    vstr.64 d27, [r0, #_bu]
...
;----------------------------------------------------------------------------
;
; void KeccakP1600_12_Pl2_PermuteAll( void *states )
;
    ALIGN
    EXPORT	KeccakP1600_12_Pl2_PermuteAll
KeccakP1600_12_Pl2_PermuteAll   PROC
